  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      // Must be removed first because student FK uses ON DELETE RESTRICT.
      await tx.classRescheduleRequest.deleteMany({
        where: { studentId: id },
      });

      // Invoice references Payment, so Invoice must be deleted before Payment.
  ...
  const sanitizeBulkDiscountTiers = (
    ...
    const sanitized = tiers.map((tier, index) => {
      ...
      // New format uses separate INR and USD discounts.
      // discountPercent is retained as fallback for old clients.
      const discountINR = Number(
        rawTier.discountINR ?? rawTier.discountPercent
      );

      const discountUSD = Number(
        rawTier.discountUSD ?? rawTier.discountPercent
      );
      ...
      if (
        !Number.isFinite(discountINR) ||
        discountINR < 0 ||
        discountINR > 100
      ) {
        throw new Error(
          `Discount tier ${index + 1}: discountINR must be between 0 and 100.`
        );
      }

      if (
        !Number.isFinite(discountUSD) ||
        discountUSD < 0 ||
        discountUSD > 100
      ) {
        throw new Error(
          `Discount tier ${index + 1}: discountUSD must be between 0 and 100.`
        );
      }
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      // Reschedule requests have a required Batch FK and block Batch deletion.
      await tx.classRescheduleRequest.deleteMany({
        where: { batchId: id },
      });
